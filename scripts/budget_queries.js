  ...
  //gets fund or department totals per year for highcharts  	
	BudgetQueries.getTotalArray = function(name, queryType, isAppropriation, callback) {
	...
	BudgetQueries.getTotalsForYear = function(name, queryType, year, callback) {
		var whereClause = "";
		if (name != "")
	...
	BudgetQueries.getAllFundsForYear = function(year, callback) {		
	...
	//returns all funds budgeted/spent totals for given year
	BudgetQueries.getDepartments = function(name, queryType, year, callback) {		
	...
	BudgetQueries.getAllControlOfficersForYear = function (year, callback) {		
	...
	BudgetQueries.getFundDescription = function(fund, callback) {
	...
	BudgetQueries.getControlOfficerDescription = function(officer, callback) {
	...
	BudgetQueries.getDepartmentDescription = function(departmentId, callback) {
	...
	BudgetQueries.getSparklinePercentages = function(name, queryType, year, callback) {	
		if (year > 1993) {
			var whereClause = "";
			if (queryType != "")
				whereClause += " WHERE '" + queryType + "' = '" + name + "'";
